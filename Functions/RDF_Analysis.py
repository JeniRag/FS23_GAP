...
def set_periodic(file_path, index=1):
    ...
    skip: frame slices indices
    
    returns ase cell, with coordinates set to periodic
    """
...
def RDF(file_path, index=1, nbins=100, rmax=None):
    """
        file_path: file
        index: slice of frames
        nbins = number of nbins
        rmax =
        save_path: path to store rdf numpy array
