...
def get_entropy(dos, mu, xdos, beta):
    """compute the electronic entropy of one srtucture
        INPUTS:
        =======
        dos: array of the DOS
        mu: Fermi energy
...
def get_finiteT_force(DOS, DOS_grad, xdos, temperature, nele=1.):
    ...
    free_energy = band_energy - temperature * entropy_T + 200 * entropy_0

    ## compute the total force
    force = grad_be_T - grad_be_0 - temperature * grad_entr_T + grad_entr_0 / (kb * beta_0)
    #force = force.reshape((self.natoms, 3))
    force = -force

    return free_energy, force